...
def prediction(model, data_df, raw_inputs, rush_hour_feature_func):
    ...
    if distance_value <= 0:
        print(
            f"\033[91mError: Invalid distance value - The value must be greater than 0 and no more than 20 miles."
        )
        return
    ...
    if time_value <= 0 or time_value > 24:
        print(
            f"\033[91mError: Invalid time value - The value must be greater than 0 and no more than 24.0."
        )
        return
    ...
    if weekend_value not in [0.0, 1.0]:
        print(
            f"\033[91mError: Invalid weekend value - Please use 1 (True) for a weekend or 0 (False) for a weekday."
        )
        return
    ...
    if time_value <= 8 or time_value > 20:
        print(
            f"\033[91mOutside Operating Hours: Deliveries are only processed between 8:00 and 20:00."
        )
        return
